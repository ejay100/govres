    ...
    <DashboardLayout title="Diaspora Portal">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-xl p-5 shadow-sm border-t-4 border-govres-gold">
          <p className="text-xs text-gray-500 uppercase">Gold Reserve</p>
          <p className="text-2xl font-bold mt-1">{fmt(r?.goldReserve?.totalOunces)} oz</p>
          <p className="text-xs text-gray-400 mt-1">{fmtCedi(r?.goldReserve?.valueCedi)}</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border-t-4 border-yellow-700">
          <p className="text-xs text-gray-500 uppercase">Cocoa Reserve</p>
          <p className="text-2xl font-bold mt-1">{fmt(r?.cocoaReserve?.totalTonnes)} MT</p>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border-t-4 border-govres-green">
          <p className="text-xs text-gray-500 uppercase">GBDC Circulation</p>
          <p className="text-2xl font-bold mt-1">{fmtCedi(r?.gbdcCirculation?.totalValue)}</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border-t-4 border-blue-500">
          <p className="text-xs text-gray-500 uppercase">eCedi Volume</p>
          <p className="text-2xl font-bold mt-1">{fmtCedi(ecedi?.totalVolume)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold mb-4">Yield Notes</h2>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center py-2 border-b">
              <div>
                <p className="font-medium text-sm">Gold Yield Note – Series A</p>
            ...
            <div className="flex justify-between items-center py-2 border-b">
              <div>
                <p className="font-medium text-sm">Cocoa Seasonal Bond – Q4 2025</p>
                <p className="text-xs text-gray-400">6-month maturity • CRDN-backed</p>
              </div>
              <span className="text-xs px-2 py-1 rounded-full bg-yellow-100 text-yellow-800 font-medium">Upcoming</span>
            </div>
            <div className="flex justify-between items-center py-2">
              <div>
                <p className="font-medium text-sm">Infrastructure Fund – Road Sector</p>
                <p className="text-xs text-gray-400">24-month maturity • GH₵-denominated</p>
              </div>
              <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800 font-medium">Open</span>
            </div>
          </div>
        ...
        {/* eCedi Gateway */}
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold mb-4">eCedi & Remittance Gateway</h2>
          <p className="text-sm text-gray-600 mb-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">eCedi Total Volume</span>
              <span className="font-medium">{fmtCedi(ecedi?.totalVolume)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Active Transactions</span>
              <span className="font-medium">{fmt(ecedi?.totalTransactions)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Exchange Rate (USD/GHS)</span>
